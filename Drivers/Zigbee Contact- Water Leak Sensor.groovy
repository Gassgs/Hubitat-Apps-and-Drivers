/**
Modified code below to turn ecolink contact sensors  into water/leak sensor
...
metadata {
	definition(name: "Ecolink Water Sensor ", namespace: "Gassgs", author: "SmartThings/Gassgs") {
		capability "Battery"
			...
			if (descMap?.clusterInt == zigbee.POWER_CONFIGURATION_CLUSTER && descMap.commandInt != 0x07 && descMap?.value) {
				...
				if (descMap.data[0] == "00") {
					...
					sendEvent(name: "checkInterval", value: 60 * 12, displayed: false, data: [protocol: "zigbee", hubHardwareId: device.hub.hardwareID, offlinePingable: "1"])
					 ...
					 if (logEnable) log.warn "IAS ZONE REPORTING CONFIG FAILED- error code: ${descMap.data[0]}"
				}
				...
				if (Integer.parseInt(descMap.value, 16) == IAS_ZONE_TYPE_CONTACT_SWITCH_ATTRIBUTE_VALUE) {
					...
					//multi-sensor is in water detector mode, DTH should be changed to water sensor DTH
					 if (logEnable) log.debug "Changing DTH type to: SmartSense Moisture Sensor"
...
private Map parseIasMessage(String description) {
	...
	return zs.isAlarm1Set() ? getContactResult('dry') : getContactResult('wet')
}
...
private Map getContactResult(value) {
	 if (logEnable) log.debug 'Contact Status'
	def linkText = getLinkText(device)
...
def refresh() {
	 if (logEnable) log.debug "Refreshing Temperature and Battery"
