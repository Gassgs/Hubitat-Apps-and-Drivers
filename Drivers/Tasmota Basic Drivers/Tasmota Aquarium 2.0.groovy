/*
 *  Tasmota Aquarium w/ Tasmota Sonoff RF Bridge, Tasmota Fish Feeder, and Tasmota S31 for Heater
 *
 * RF Controlled Light, Tasmota Fish Feeder with added Temperature Probe , Tasmota S31 for Heater Control
 *
 ...
 * Tasmota Sonoff S31 for Heater Control
 *
 *
 ...
 *  V2.0.0  08-23-2022       Added Heater control via Tasmota sonoff S31 plug//version jump to 2.0
 *
 */
def driverVer() { return "2.0" }

metadata {
    definition(name: "Tasmota Aquarium 2.0", namespace: "Gassgs ", author: "Gary G") {
        capability "Actuator"
        capability "Switch"
        ...
        command "blueLight", [[name:"blueLight", type: "ENUM",description: "Blue Light", constraints: ["Off","High","Low"]]]
        command "heater", [[name:"heater", type: "ENUM",description: "Heater", constraints: ["On", "Off",]]]
        command  "feedFish"
        ...
        attribute "wifi1","string"
        attribute "wifi2","string"
    }
}

preferences {
    section("URIs") {
		input "sonoffIP", "text", title: "<b>Sonoff RF Bridge IP Address</b>", required: true
        input "feederIP", "text", title: "<b>Tasmota Feeder IP Address</b>", required: true
        input "heaterIP", "text", title: "<b>Tasmota Heater IP Address</b>", required: true
        input name: "hubIp",type: "string", title: "<b>Hubitat Device IP Address</b>", required: true
        input name: "tempSet", type: "number" , title: "<b>Aquarium Temperature Setting</b>", defaultValue: 80
           ...
           def json = (resp.data)
            if (logEnable) log.debug "${json}"
               ...
               if (json.containsKey("StatusSTS")){
                   ...
                   if (logEnable) log.debug "Wifi1 signal strength $signal db"
                   sendEvent(name:"wifi1",value:"${signal}db")
                   if (logEnable) log.debug "$device.label $feederIP - $status Wifi1 signal strength $signal db"
                   if (infoEnable) log.info "$device.label Wifi1 signal strength $signal db"
               }
           }
    } catch (Exception e) {
            sendEvent(name:"wifi1",value:"offline")
            log.warn "Call to on failed: ${e.message}"
        }
    }
    refresh2()
...
def heaterOn() {
    if (logEnable) log.debug "Sending On Command to [${settings.heaterIP}]"
    try {
        httpGet("http://" + heaterIP + "/cm?cmnd=POWER%20On") { resp ->
            def json = (resp.data)
            if (logEnable) log.debug "${json}"
            if (json.POWER == "ON"){
                if (logEnable) log.debug "Command Success response from Device"
...
def heaterOff() {
    if (logEnable) log.debug "Sending Off Command to [${settings.heaterIP}]"
    try {
        httpGet("http://" + heaterIP + "/cm?cmnd=POWER%20Off") { resp ->
            def json = (resp.data)
            if (logEnable) log.debug "${json}"
            if (json.POWER == "OFF"){
                if (logEnable) log.debug "Command Success response from Device"
...
def refresh2(){
        if(settings.heaterIP){
        if (logEnable) log.debug "Refreshing Device Status - [${settings.heaterIP}]"
        try {
           httpGet("http://" + heaterIP + "/cm?cmnd=status%200") { resp ->
           def json = (resp.data)
            if (logEnable) log.debug "${json}"
               if (json.containsKey("StatusSTS")){
                   status = json.StatusSTS.POWER as String
                   signal = json.StatusSTS.Wifi.Signal as String
                   if (logEnable) log.debug "Wifi2 signal strength $signal db"
                   sendEvent(name:"wifi2",value:"${signal}db")
                   if (logEnable) log.debug "$device.label $heaterIP - $status Wifi2 signal strength $signal db"
                   if (infoEnable) log.info "$device.label Heater $status - Wifi2 signal strength $signal db"
                   if (status == "ON"){
            ...
            sendEvent(name:"wifi2",value:"offline")
            log.warn "Call to on failed: ${e.message}"
        }
    }
}
...
def heatCheck(){
    ...
    heatStatus = device.currentValue("heater")
        if (heatStatus == "on"){
            state.heatOn = true
        }else{
            state.heatOn = false
        }
        tempLow = (settings.tempSet - 1)
        if (currentTemp >= tempSet && state.heatOn){
            if (logEnable) log.debug "$device.label Current temp is $currentTemp, temp set to $tempSet Turning Heater Off"
            if (infoEnable) log.info "$device.label Current temp is $currentTemp, temp set to $tempSet Turning Heater Off"
            heaterOff()
        }
        else if (currentTemp <= tempLow && !state.heatOn){
            if (logEnable) log.debug "$device.label Current temp is $currentTemp, temp set to $tempSet Turning Heater On"
            if (infoEnable) log.info "$device.label Current temp is $currentTemp, temp set to $tempSet Turning Heater On"
            heaterOn()
        }
}
