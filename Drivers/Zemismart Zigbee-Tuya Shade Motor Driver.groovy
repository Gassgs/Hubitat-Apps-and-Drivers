/** Zemismart  Tuya/Zigbee Shade Motor Driver
 *  for model ZM85EL-1Z
 ...
 *  Tuya Window Shade (v.1.3.0) Hubitat v4 added battery level for new motor Gassgs
...
metadata {
	definition(name: "Zemismart Zigbee-Tuya Shade Motor", namespace: "ShinJjang/Gassgs", author: "ShinJjang-iquix") {
		capability "Battery"
        capability "Actuator"
        ...
        fingerprint  profileId:"0104",inClusters:"0000,0004,0005,EF00",outClusters:"0019,000A",manufacturer:"_TZE200_68nvbio9",model:"TS0601",deviceJoinName:"Zemismart Zigbee Shade"
		...
		if (descMap?.clusterInt==CLUSTER_TUYA) {
			if ( descMap?.command == "01" || descMap?.command == "02" ) {
				...
				switch (dp) {
                    ...
                    case 525: // Battery
                    	def bat = zigbee.convertHexToInt(descMap.data[9])
                        if(logEnable) log.debug description
                    	if(logInfoEnable) log.info "$device.label Battery Level:"+bat+"%"
