/**
 *  Garage Door -  Z-Wave & Tasmota 
 *  
 *  Author: Smartthings
 *    Mod by Gassgs
 *
 *  Ecolink Tilt sensor Gen 5 w/ Tasmota relay to "push" the garage door button
 *
 *
...
metadata {
	definition (
		name: "Garage Door - Z-Wave & Tasmota", namespace: "Gassgs", author: "SmartThings"
	) {
        capability "Actuator"
	...
	preferences {
        ...
        input "timeout", "number", title: "Motion timeout threshold in minutes", defaultValue: 5, required: true, multiple: false
        ...
        input name: "refreshEnable",type: "bool", title: "<b>Enable to ping Tasmota device every 30mins</b>", defaultValue: true
...
def open(){
    if (state.door == "closed"){
        ...
        if (infoEnable) log.info "$device.label Garage Door is already open - not pushing button"
    }      
}
...
def close(){
    if (state.door == "open"){
        ...
        if (infoEnable) log.info "$device.label Garage Door is already closed - not pushing button"
    }   
}
    ...
    try {
        httpGet("http://" + deviceIp + "/cm?cmnd=POWER%20On") { resp ->
            ...
            if (json.POWER == "ON"){
                ...
                if (infoEnable) log.info "$device.label Garage Door is opening"
    ...
    try {
        httpGet("http://" + deviceIp + "/cm?cmnd=POWER%20On") { resp ->
            ...
            if (json.POWER == "ON"){
                ...
                if (infoEnable) log.info "$device.label Garage Door is closing"
                if (logEnable) log.debug "Command Success response from Device"
            }else{
                if (logEnable) log.debug "Command -ERROR- response from Device- $json"
            }
           ...
           def json = (resp.data)
            if (logEnable) log.debug "${json}"
               if (json.containsKey("StatusSTS")){
                   if (logEnable) log.debug "PWR & Wifi status found"
            ...
            if (infoEnable) log.error "$device.label Unable to connect, device is <b>OFFLINE</b>"
            log.warn "Call to on failed: ${e.message}"
        }
    }
}
	...
	switch (cmd.parameterNumber) {
		case 1:
			state.enableExternalSensor = (cmd.configurationValue[0] == 0xFF)
			  if (logEnable) log.debug "External Sensor Enabled: ${state.enableExternalSensor}"
			break
		default:	
...
def zwaveEvent(hubitat.zwave.commands.notificationv3.NotificationReport cmd) {
    ...
    if (cmd.v1AlarmType == 0x03){
        result += handleTamperEvent(cmd.event)
    }
    else if (cmd.v1AlarmType == 0x02){
...
private handleContactEvent(event) {
	if (event == 2) {
        ...
        if (infoEnable) log.info "$device.label contact open"
        runIn(timeout*60,reset)
	}
	else if(event == 0) {
        unschedule(reset)
        state.door = "closed"
        ...
        if (infoEnable) log.info "$device.label contact closed"
	}
}

def reset(){
    sendEvent(name:"motion",value:"inactive")
    if (infoEnable) log.info "$device.label motion inactive after timeout"
}
