...
0.9.3 - tomw - Added support for Microwave
...
metadata 
{
    definition(name: "SmartHQ Microwave", namespace: "tomw", author: "tomw") 
    {
        ...
        command "adjustMicrowaveState", [[name: "Cook status*", type:"ENUM", description:"Cook status mode to set", constraints:["on", "off", "paused"]]]
        //command "startTimer",[[name: "seconds*", type: "NUMBER"]]
        command "start"
        command "stop"
        command "pause"
...
def cookModeNames()
{
    cookModes.collect { it.value.toString() }
}

def cookStatusNames()
{
    cookStatuses.collect { it.value.toString() }    
}
...
def parseMicrowaveState(value)
{
    try
    {
        ...
        status = device.currentValue("cookStatus")
        log.info "$device.label - Cook status is - '$status'"
        if (status == "off"){
...
def on(){
    adjustMicrowaveState("on")
}

def off(){
    adjustMicrowaveState("off")
}

def pause(){
    adjustMicrowaveState("paused")
}
...
def startTimer(sec){
    log.warn " is this minutes or seconds?? $sec"   
}
